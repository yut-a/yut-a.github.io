---
layout: post
title:  코로나 상황에서 재무비율 특성에 따른 Kospi100 종목들의 주가 충격과 회복
date:   2020-10-07
...
이처럼 거의 모든 종목들이 급락을 면치 못하는 상황에서도, 하락률의 차이가 있을 것이라 생각했고, 이에 대한 차이를 나타내는 요인이 기업의 `부채 비중`, `자기자본 비중` 등 재무 상태가 아닐까 하는 궁금증이 생겼다. 따라서, 코로나와 같은 경제 위기에서 주요 재무 비율의 특성에 따라 **주가 하락 충격이 어떻게 달라지는지**를 알아보고자 한다. 또한, 재무 비율 특성에 따라 주가 급락 이후 현재까지 **회복 기조가 어떻게 나타나는지** 역시 알아보고자 한다.<BR/><BR/><BR/><BR/>

## 데이터 소개

`Kospi100 구성 종목 중 78개의 데이터`를 활용했다. Kospi100의 구성 종목은 100개이나 78개의 데이터를 사용한 이유는 두 가지이다.
...
종목 별 재무비율 데이터에 종목명 칼럼을 추가하고 결측치를 제거했다. 결측치가 존재하는 6개의 종목들을 제거했다.
...
기업의 주요 재무비율 중 **안정성 지표가 좋을** 수록, 코로나 19와 같은 위기 상황에서 **낮은 주가 하락률**을 보일 가능성이 크다. 기업의 주요 재무비율 중 **안정성과 성장성 지표 모두 나쁠** 경우, 코로나 19와 같은 위기 상황에서 **높은 주가 하락률**을 보일 가능성이 크다. 따라서, 경제 위기의 상황에서 `기업`의 경우, 기업 가치 급락을 방지하기 위해 **현금성 자산, 자기자본 확충 등 안정성 지표를 견고히 하는데 집중**해야 하며, `투자자`의 경우, 투자 포트폴리오 가치 급락을 방지하기 위해 **안정성 지표가 좋은 종목에 투자**하는 것이 바람직하다.

* 기업의 주요 재무비율 중 성장성 지표의 좋고 나쁨은 코로나 19와 같은 위기 상황에서 하락률 수준과 큰 관련이 없다.
* 코로나 충격으로 인한 회복률 수준은 기업의 주요 재무비율의 특성과 큰 관련이 없다.

#### _그 외_

* 이번 분석에서는 적정량의 데이터와 설명력을 위해 Kospi100 구성 종목을 활용했다. 데이터의 선택 과정에서 Kopsi50, 100, 200으로 종목 수를 늘려갈 경우, PCA(n=3)를 통한 설명력이 88%, 81%, 72%로 감소한다는 것을 알 수 있었다. 이는 기업 규모가 작아질 수록 재무 지표 외적인 요인들에 더 많은 영향을 받을 수 있다고 예상해 볼 수 있다.
* 안정성, 성장성 지표 모두 나쁜 종목 중 낮은 주가 하락률을 보인 종목의 특징을 파악하고자 했다. 마찬가지로, 지표가 대체로 좋은 종목 중 높은 주가 하락률을 보인 종목의 특징을 파악하고자 했다. 그러나, 산업 특성, 이슈, 수익률 지표 수준 등 별다른 특징은 존재하지 않았다.<BR/><BR/><BR/><BR/>

## 한계

* 활용한 종목 수의 데이터가 적어 일반화하여 적용하기 어렵다.
* 이전 경제 위기의 데이터를 함께 활용한 것이 아니기 때문에 코로나 19 경제 위기 상황에만 해당된다.
* 수익률 지표에 따른 특성을 제대로 파악할 수 없었다.
