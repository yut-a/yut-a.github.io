...
`<font color="#C66104">Bayesian</font>`은 확률을 현상에 대한 관찰자의 주관적 믿음의 체계로 본다. 즉, `Bayesian`은 과거의 사건을 바탕으로 현재의 사건을 판단한다. `Bayesian`은 데이터를 얻기 전에 분포에 대한 믿음인 `사전확률분포`를 가지고 있다. 그리고 데이터를 얻게 되면, 과거의 자신의 견해를 갱신하여 새로운 믿음인 `사후확률분포`를 만들고 이러한 과정에 따라 참된 분포에 접근시킨다.
